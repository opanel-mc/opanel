package net.opanel.forge_1_21_11.utils;
    ...
    public static HashMap<String, Object> serializeNBT(CompoundTag nbt) {
        ...
        for(String key : nbt.keySet()) {
            ...
            switch(Objects.requireNonNull(nbtElem).getId()) {
                case Tag.TAG_BYTE -> {
                    if(nbtElem instanceof ByteTag) obj.put(key, ((ByteTag) nbtElem).byteValue());
                }
                case Tag.TAG_SHORT -> {
                    if(nbtElem instanceof ShortTag) obj.put(key, ((ShortTag) nbtElem).shortValue());
                }
                case Tag.TAG_INT -> {
                    if(nbtElem instanceof IntTag) obj.put(key, ((IntTag) nbtElem).intValue());
                }
                case Tag.TAG_LONG -> {
                    if(nbtElem instanceof LongTag) obj.put(key, ((LongTag) nbtElem).longValue());
                }
                case Tag.TAG_FLOAT -> {
                    if(nbtElem instanceof FloatTag) obj.put(key, ((FloatTag) nbtElem).floatValue());
                }
                case Tag.TAG_DOUBLE -> {
                    if(nbtElem instanceof DoubleTag) obj.put(key, ((DoubleTag) nbtElem).doubleValue());
                }
    ...
    public static List<Object> serializeNBTList(ListTag nbtList) {
        ...
        for(Tag nbtElem : nbtList) {
            switch(nbtElem.getId()) {
                case Tag.TAG_INT -> {
                    if(nbtElem instanceof IntTag) list.add(((IntTag) nbtElem).intValue());
                }
                case Tag.TAG_SHORT -> {
                    if(nbtElem instanceof ShortTag) list.add(((ShortTag) nbtElem).shortValue());
                }
                case Tag.TAG_FLOAT -> {
                    if(nbtElem instanceof FloatTag) list.add(((FloatTag) nbtElem).floatValue());
                }
                case Tag.TAG_DOUBLE -> {
                    if(nbtElem instanceof DoubleTag) list.add(((DoubleTag) nbtElem).doubleValue());
                }
                case Tag.TAG_STRING -> {
