...
/**
 * Forge 1.21.11 implementation of ItemDataResolver.
 * Same API as 1.21.5+ (getArmor, getOffhandItem methods).
 */
public class Forge12111ItemDataResolver implements ItemDataResolver {
    ...
    public Map<String, Object> serializeItem(ItemStack item) {
        ...
        if (loreComponent != null) {
            ...
            for (Component line : loreComponent.lines()) loreList.add(line.getString());
        ...
        if (enchantments != null && !enchantments.isEmpty()) {
            ...
            for (Holder<Enchantment> entry : enchantments.keySet()) {
                entry.unwrapKey().ifPresent(key -> enchantMap.put(key.location().getPath(), enchantments.getLevel(entry.value())));
            }
            if (!enchantMap.isEmpty()) result.put("enchantments", enchantMap);
        }

        if (item.has(DataComponents.UNBREAKABLE)) result.put("unbreakable", true);
        return result;
    }
    ...
    public List<ItemStack> getArmorItems(ServerPlayer player, Inventory inventory) {
        ...
        armor.add(inventory.getArmor(3));
        armor.add(inventory.getArmor(2));
        armor.add(inventory.getArmor(1));
        armor.add(inventory.getArmor(0));
        return armor;
    }
    ...
    @Override
    public String getTargetVersion() { return "1.21.11"; }
}
