...
public class FabricListener {
    ...
    public FabricListener() {
        ServerPlayConnectionEvents.JOIN.register((networkHandler, sender, server) -> {
            ...
            server.execute(() -> { if (inventorySyncTask != null && player != null) inventorySyncTask.syncPlayer(player); });
        });
    ...
    public void emitInventoryChange(ServerPlayerEntity player) {
        ...
        EventManager.get().emit(EventType.PLAYER_INVENTORY_CHANGE, new OPanelPlayerInventoryChangeEvent(new FabricPlayer(player, server), inventoryData));
    }
}
