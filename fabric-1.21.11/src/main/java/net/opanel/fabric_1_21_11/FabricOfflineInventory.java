...
public class FabricOfflineInventory extends BaseFabricOfflineInventory {
    ...
    public List<OPanelItemStack> getItems() {
        ...
        for(int i = 0; i < nbtList.size(); i++) {
            ...
            int slot = toPanelSlot(itemNbt.getByte("Slot", (byte) 0));
            if(!isValidMainSlot(slot)) continue;

            String id = itemNbt.getString("id", "minecraft:air");
            int count = itemNbt.getByte("count", (byte) 0);
            Optional<NbtCompound> components = itemNbt.getCompound("components");
            items.set(slot, new OPanelItemStack(slot, id, count, components.map(NbtCompound::toString).orElse(null)));
        }
    ...
    public List<OPanelItemStack> getEnderItems() {
        ...
        for(int i = 0; i < enderNbtList.size(); i++) {
            NbtCompound itemNbt = enderNbtList.getCompoundOrEmpty(i);
            int slot = normalizeRawSlot(itemNbt.getByte("Slot", (byte) 0));
            if(slot < 0 || slot >= ENDER_SLOT_COUNT) continue;

            String id = itemNbt.getString("id", "minecraft:air");
            int count = itemNbt.getByte("count", (byte) 0);
            Optional<NbtCompound> components = itemNbt.getCompound("components");
            items.set(slot, new OPanelItemStack(slot, id, count, components.map(NbtCompound::toString).orElse(null), "ender"));
        }
    ...
    public void setItems(List<OPanelItemStack> items) throws CommandSyntaxException {
        try {
            for(int i = nbtList.size() - 1; i >= 0; i--) {
                int slot = toPanelSlot(nbtList.getCompoundOrEmpty(i).getByte("Slot", (byte) 0));
    ...
    public void setEnderItem(OPanelItemStack item) throws CommandSyntaxException {
        try {
            ...
            for(int i = enderNbtList.size() - 1; i >= 0; i--) {
                int slot = normalizeRawSlot(enderNbtList.getCompoundOrEmpty(i).getByte("Slot", (byte) 0));
                if(slot == item.slot) {
