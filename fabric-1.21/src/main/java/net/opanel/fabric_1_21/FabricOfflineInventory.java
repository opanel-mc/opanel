...
public class FabricOfflineInventory extends BaseFabricOfflineInventory {
    ...
    public List<OPanelItemStack> getItems() {
        ...
        for(int i = 0; i < nbtList.size(); i++) {
            ...
            NbtCompound components = itemNbt.getCompound("components");
            items.set(slot, new OPanelItemStack(slot, id, count, components.isEmpty() ? null : components.toString()));
        }
    ...
    public List<OPanelItemStack> getEnderItems() {
        ...
        for(int i = 0; i < enderNbtList.size(); i++) {
            ...
            int count = itemNbt.getByte("count");
            NbtCompound components = itemNbt.getCompound("components");
            items.set(slot, new OPanelItemStack(slot, id, count, components.isEmpty() ? null : components.toString(), "ender"));
        }
